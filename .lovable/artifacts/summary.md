...
## Files
- `seo-report.json` — Aggregated report
- `seo-history.json` — Regression & fix history
- `seo-baseline.json` — Metadata baseline
- `indexing-state.json` — Indexing verdicts
- `indexing-cache.json` — Indexing API cache
- `lighthouse.json` — Lighthouse raw run
- `link-check.csv` — Link check table
- `lighthouse-thresholds.json` — Lighthouse thresholds
- `alerts-log.json` — Alert delivery log
